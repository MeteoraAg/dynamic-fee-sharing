...
describe("Fee vault sharing", () => {
  ...
  describe("Validation", () => {
    it("Fail to create more than max user", async () => {
      ...
      const res = await initializeFeeVault({
        svm,
        program,
        feeVault,
        tokenMint,
    ...
    it("Fail to update operator when fee vault is not mutable", async () => {
      ...
      const initializeFeeVaultRes = await initializeFeeVault({
        svm,
        program,
        feeVault,
        tokenMint,
  ...
  describe("initialized mutable fee vault", () => {
    ...
    beforeEach(async () => {
      ...
      const initRes = await initializeFeeVault({
        svm,
        program,
        feeVault,
        tokenMint,
    ...
    it("Remove, re-add and remove user again accumulates unclaimed fees", async () => {
      ...
      const reAddedIndex = getFeeVault(svm, feeVault.publicKey).users.findIndex(
        (u) => u.address.equals(removedUser.publicKey),
      );
      svm.expireBlockhash();
      await removeUser({
        svm,
    ...
    it("Full flow", async () => {
      ...
      const beforeFeePerShare = getFeeVault(
        svm,
        feeVault.publicKey,
      ).feePerShare;
      ...
      const newUserIndex = getFeeVault(svm, feeVault.publicKey).users.findIndex(
        (u) => u.address.equals(newUser.publicKey),
      );
